...
import 'package:gstockdz/widgets.dart/custom_drop_down_menu.dart';
import'package:gstockdz/widgets.dart/custom_Input_Settings.dart';
import'package:gstockdz/widgets.dart/Image_Upload.dart';
import'package:gstockdz/widgets.dart/switch_Title.dart';
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 30),
                  CustomDropMenu(label: 'الدولة', items:countries, onChanged:  (value) {
            setState(() {
              ...
              CustomDropMenu(label: 'اللغة', items:language, onChanged:  (value) {
            setState(() {
...
SizedBox(height: 20,),
 ...
 CustomDropMenu(label: 'عدد الاجزاء', items:joz2, onChanged:  (value) {
            setState(() {
              selectedJoz2 = value;
            });
...
SizedBox(height: 20,),
ImageUpload(label1: 'تغيير',label2: 'صورة الشعار',),
SizedBox(height: 20,),

 CustomDropMenu(label: 'حجم الشعار', items:size, onChanged:  (value) {
            setState(() {
              selectedSize = value;
            });
...
const SizedBox(height: 20),
              CustomSwitch(
  action: true, // Initial state of the switch
  label: 'السماح بالتخفيض للرصيد عند تغيير سعر الصنف للعملة الأعلى',
),
const SizedBox(height: 20),
              CustomSwitch(
  action: true, // Initial state of the switch
...
const SizedBox(height: 20),
               CustomSwitch(
  action: true, // Initial state of the switch
  label: 'تغيير أسعار العملات الأصغر لها عند تغيير سعر العملة للعملة الأعلى',
),
const SizedBox(height: 20),
               CustomSwitch(
  action: true, // Initial state of the switch
...
const SizedBox(height: 20),
  ...
  label: 'اظهار مورد المنتج في شاشة البيع-المرد',
          ...
          const SizedBox(height: 20),
CustomDropMenu(label: 'حجم صورة المنتج في شاشة العرض', items:sizePic, onChanged:  (value) {
            setState(() {
              selectedSizPic = value;
            });
          }, selectedValue: selectedSizPic),
                    const SizedBox(height: 20),
CustomDropMenu(label: 'عدد الاعمدة في قائمة المنتجات', items:number, onChanged:  (value) {
            setState(() {
